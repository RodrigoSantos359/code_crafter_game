...
"></div>
      ...
      <div className="relative min-h-screen p-6">
        {/* Header */}
        <div className="max-w-6xl mx-auto mb-8 dark:text-slate-100">
          <Button
            variant="ghost"
            onClick={onBack}
          ...
          <div className="text-center mb-8 dark:text-slate-100">
            <div className="text-6xl mb-4">📚</div>
            <h1 className="text-5xl font-bold text-slate-800 dark:text-slate-100 mb-2">Tutoriais</h1>
            <p className="text-xl text-slate-800 dark:text-slate-100 max-w-2xl mx-auto">
        ...
        {/* Lista de Tutoriais */}
        <div className="max-w-6xl mx-auto space-y-6 dark:text-slate-100">
          {tutorials.map((tutorial, index) => (
            <Card key={tutorial.id} className="p-6 hover:shadow-lg transition-all dark:bg-slate-900/50 dark:border-slate-600/50">
              <div className="flex flex-col md:flex-row gap-6">
                {/* Ícone e Título */}
                <div className="flex-shrink-0">
                  <div className="text-6xl mb-4 text-center md:text-left text-slate-800 dark:text-slate-100">{tutorial.icon}</div>
                  <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">{tutorial.title}</h2>
                  <p className="text-slate-800 dark:text-slate-100 mb-4">{tutorial.description}</p>
                </div>
                ...
                <div className="flex-1 space-y-4">
                  <div className="space-y-2">
                    {tutorial.content.map((paragraph, pIndex) => (
                      <p key={pIndex} className="text-slate-800 dark:text-slate-100 leading-relaxed">
                        {paragraph}
                      </p>
                    ))}
                  </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="bg-slate-900 dark:bg-slate-100 text-green-400 p-4 rounded-lg font-mono text-sm overflow-x-auto">
        ...
        <div className="max-w-6xl mx-auto mt-12 text-center">
          <Card className="p-8 bg-gradient-to-r from-blue-600 to-indigo-600 text-white dark:text-slate-800">
            <h3 className="text-2xl font-bold mb-4">Pronto para praticar?</h3>
            <p className="text-blue-100 dark:text-slate-800 mb-6">
              Agora que você conhece os conceitos básicos, teste seus conhecimentos no jogo!
            </p>
            <Button
              onClick={onBack}
              className="bg-white dark:bg-slate-800 text-blue-600 hover:bg-blue-50 gap-2"
            >
              Voltar ao Menu
